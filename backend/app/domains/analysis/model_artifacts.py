...
def artifact_paths_from_env() -> ModelArtifactPaths:
    ...
    if not configured_dir:
        raise AppError(
            ...
            message="서버에 분석 모델 경로가 설정되지 않았습니다. 관리자에게 문의해 주세요.",
...
def require_artifacts(
    ...
    if missing:
        raise AppError(
            ...
            message="서버에 분석에 필요한 모델 파일이 없습니다. 관리자에게 문의해 주세요.",
...
def load_ensemble_stats(paths: ModelArtifactPaths) -> dict[str, float]:
    ...
    if missing_keys:
        raise AppError(
            ...
            message="서버의 모델 기준 정보에 필수 항목이 없습니다. 관리자에게 문의해 주세요.",
    ...
    try:
        ...
        raise AppError(
            ...
            code="MODEL_ARTIFACT_INVALID",
            message=(
                "서버의 모델 기준 정보에 올바르지 않은 값이 있습니다. 관리자에게 문의해 주세요."
            ),
    ...
    with paths.gene_coefficients.open(encoding="utf-8-sig", newline="") as file:
        ...
        if reader.fieldnames != ["significant_genes", "coef"]:
            raise AppError(
                ...
                code="MODEL_ARTIFACT_INVALID",
                message=(
                    "서버의 유전자 분석 기준 파일 형식이 올바르지 않습니다. "
                    "관리자에게 문의해 주세요."
                ),
        ...
        for row_index, row in enumerate(reader, start=2):
            ...
            if not gene or not raw_coef:
                raise AppError(
                    ...
                    code="MODEL_ARTIFACT_INVALID",
                    message=(
                        "서버의 유전자 분석 기준 파일에 누락된 값이 있습니다. 관리자에게 "
                        "문의해 주세요."
                    ),
            ...
            try:
                ...
                raise AppError(
                    ...
                    code="MODEL_ARTIFACT_INVALID",
                    message=(
                        "서버의 유전자 분석 기준 파일에 숫자가 아닌 값이 있습니다. "
                        "관리자에게 문의해 주세요."
                    ),
...
def load_ordered_feature_coefficients(paths: ModelArtifactPaths) -> list[tuple[str, float]]:
        ...
        for row_index, row in enumerate(reader, start=2):
            ...
            if not name:
                raise AppError(
                    ...
                    code="MODEL_ARTIFACT_INVALID",
                    message=(
                        "서버의 유전자 분석 기준 파일에 누락된 항목 이름이 있습니다. "
                        "관리자에게 문의해 주세요."
                    ),
            ...
            if not math.isfinite(coefficient):
                raise AppError(
                    ...
                    code="MODEL_ARTIFACT_INVALID",
                    message=(
                        "서버의 유전자 분석 기준 파일에 계산할 수 없는 값이 있습니다. "
                        "관리자에게 문의해 주세요."
                    ),
    ...
    if len(ordered) != 293 or [name for name, _ in ordered[-5:]] != expected_tail:
        raise AppError(
            ...
            code="MODEL_ARTIFACT_INVALID",
            message=(
                "서버의 유전자 분석 기준 파일이 현재 모델과 맞지 않습니다. "
                "관리자에게 문의해 주세요."
            ),
    ...
    with paths.km_data.open(encoding="utf-8-sig", newline="") as file:
        ...
        if reader.fieldnames != ["time", "event", "ensemble_risk", "risk_group"]:
            raise AppError(
                ...
                code="MODEL_ARTIFACT_INVALID",
                message=(
                    "서버의 생존 곡선 기준 파일 형식이 올바르지 않습니다. 관리자에게 문의해 주세요."
                ),
