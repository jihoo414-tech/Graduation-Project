...
def soft_delete_analysis_result(access_token: str, result_id: str) -> bool:
    try:
        _, raw_payload = supabase_client.request(
            "/rest/v1/rpc/soft_delete_analysis_result",
            method="POST",
            access_token=access_token,
            body={"target_result_id": result_id},
        )
        ...
        raise AppError(
            ...
            message="분석 결과를 삭제하지 못했습니다. 잠시 후 다시 시도해 주세요.",
            details=[error_detail("analysis_result", "soft_delete_failed")],
        ) from exc
