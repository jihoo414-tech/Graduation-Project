...
from app.domains.analysis.model_artifacts import (
    ModelArtifactPaths,
    load_ensemble_stats,
    load_km_rows,
...
def run_ensemble(features: Sequence[float], paths: ModelArtifactPaths) -> EnsembleScores:
    if len(features) != 293 or not all(math.isfinite(value) for value in features):
        raise AppError(
            ...
            message=(
                "모델에 전달할 분석 데이터가 올바르게 구성되지 않았습니다. "
                "관리자에게 문의해 주세요."
            ),
...
def _load_runtime(paths: ModelArtifactPaths) -> _Runtime:
    ...
    try:
        ...
        raise AppError(
            ...
            message="서버의 모델 실행 기능이 준비되지 않았습니다. 관리자에게 문의해 주세요.",
    ...
    try:
        ...
        raise AppError(
            ...
            message="서버에서 생존 예측 모델을 불러오지 못했습니다. 관리자에게 문의해 주세요.",
    ...
    if not hasattr(rsf_model, "predict"):
        raise AppError(
            status_code=503,
            code="MODEL_ARTIFACT_INVALID",
            message="서버의 생존 예측 모델 파일을 사용할 수 없습니다. 관리자에게 문의해 주세요.",
    ...
    try:
        ...
        raise AppError(
            ...
            code="MODEL_ARTIFACT_INVALID",
            message=(
                "서버에서 심층 생존 예측 모델을 불러오지 못했습니다. 관리자에게 문의해 주세요."
            ),
    ...
    try:
        ...
        raise AppError(
            ...
            code="MODEL_ARTIFACT_INVALID",
            message=(
                "서버의 심층 생존 예측 모델 파일이 현재 분석 설정과 맞지 "
                "않습니다. 관리자에게 문의해 주세요."
            ),
...
def _derive_risk_threshold(rows: list[dict[str, str]]) -> float:
    ...
    candidates = [
        (left + right) / 2 for left, right in zip(unique_scores, unique_scores[1:], strict=False)
    ]
    perfect = [
        candidate
...
def _runtime_error(field: str, rule: str) -> AppError:
    return AppError(
        ...
        code="MODEL_INFERENCE_FAILED",
        message=(
            "모델이 유효한 분석 결과를 계산하지 못했습니다. 다시 시도하고 "
...
def _threshold_error(rule: str) -> AppError:
    return AppError(
        ...
        message="위험군 분류에 필요한 기준값을 확인하지 못했습니다. 관리자에게 문의해 주세요.",
